...
class EInkDisplay : public OLEDDisplay
{
  ...
  public:
    ...
    */
    ...
    /**
     * Partial window update — writes only (x,y,w,h) region from buffer to display.
     * Much faster than full refresh. Does NOT call hibernate() so the panel stays ready
     * for subsequent partial updates.
     */
    void partialWindow(int16_t x, int16_t y, int16_t w, int16_t h);

    /**
     * Static helper for partial window updates from modules that only have an OLEDDisplay*.
     * No-op if no EInkDisplay instance exists.
     */
    static void triggerPartial(int16_t x, int16_t y, int16_t w, int16_t h);
