...
class Screen : public concurrency::OSThread
{
  ...
  private:
    ...
    /// True while the UI is animating a frame transition (IN_TRANSITION).
    /// showFrame() stores the requested direction here instead of dropping it.
    bool hasPendingFrame = false;
    FrameDirection pendingFrameDirection;

    /// When set, Screen::runOnce() will push a display refresh after the next
    /// frame is rendered, bypassing the E-Ink rate limiter.
    ...
    /// For E-Ink: immediately render the current frame and push to display, bypassing rate limiter.
    /// On other display types this is a no-op.
    void einkForceUpdate();
