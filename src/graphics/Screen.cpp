...
void Screen::forceDisplay(bool forceUiUpdate)
{
    ...
    if (forceUiUpdate) {
        ...
        // Schedule the refresh for Screen::runOnce() instead of busy-waiting.
        // Previously this blocked all other OSThreads on Core 0 for ~324ms.
        setFastFramerate();
        pendingDisplayRefresh = true;
        return;
    }

    // Tell EInk class to update the display (rate-limited path)
...
int32_t Screen::runOnce()
{
    ...
    // Process pending frame navigation once transition completes
    if (hasPendingFrame && ui->getUiState()->frameState == FIXED) {
        hasPendingFrame = false;
        showFrame(pendingFrameDirection);
    }

    // Process pending display refresh once frame is fully rendered
    if (pendingDisplayRefresh && ui->getUiState()->frameState == FIXED) {
        pendingDisplayRefresh = false;
#if defined(USE_EINK) && !defined(USE_EINK_DYNAMICDISPLAY) && !defined(USE_EINK_PARALLELDISPLAY)
...
void Screen::showFrame(FrameDirection direction)
{
    ...
    if (ui->getUiState()->frameState == FIXED) {
        ...
        // Frame is still in transition — queue for execution once FIXED.
        // Previously this case was silently dropped, causing "key does nothing" stalls.
        hasPendingFrame = true;
        pendingFrameDirection = direction;
    }
}
...
void Screen::einkForceUpdate()
{
    // Non-blocking: schedule the refresh for Screen::runOnce().
    // Previously this busy-waited for the transition to complete,
    // blocking all other OSThreads on Core 0 for ~324ms.
    setFastFramerate();
    pendingDisplayRefresh = true;
}
