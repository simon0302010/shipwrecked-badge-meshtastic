...
EInkDisplay *EInkDisplay::s_instance = nullptr;

void EInkDisplay::partialWindow(int16_t x, int16_t y, int16_t w, int16_t h)
{
    if (!adafruitDisplay || w <= 0 || h <= 0)
        return;
    adafruitDisplay->setPartialWindow(x, y, w, h);
    for (int32_t py = y; py < y + h; py++) {
        for (int32_t px = x; px < x + w; px++) {
            auto b = buffer[px + (py / 8) * displayWidth];
            auto isset = b & (1 << (py & 7));
            adafruitDisplay->drawPixel(px, py, isset ? GxEPD_BLACK : GxEPD_WHITE);
        }
    }
    adafruitDisplay->nextPage();
    // Don't hibernate — keep controller awake for subsequent partial updates
}

void EInkDisplay::triggerPartial(int16_t x, int16_t y, int16_t w, int16_t h)
{
    if (s_instance)
        s_instance->partialWindow(x, y, w, h);
}
...
#elif defined(SHIPWRECKED)
    {
        s_instance = this;
        auto lowLevel = new EINK_DISPLAY_MODEL(PIN_EINK_CS, PIN_EINK_DC, PIN_EINK_RES, PIN_EINK_BUSY, SPI);
        adafruitDisplay = new GxEPD2_BW<EINK_DISPLAY_MODEL, EINK_DISPLAY_MODEL::HEIGHT>(*lowLevel);
        adafruitDisplay->init(115200, true, 40, false);
        adafruitDisplay->setRotation(0);
        adafruitDisplay->setPartialWindow(0, 0, EINK_WIDTH, EINK_HEIGHT);
        // Full clear to eliminate any ghosting from previous images (boot logo, etc.)
