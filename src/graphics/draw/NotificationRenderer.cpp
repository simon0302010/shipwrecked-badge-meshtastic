...
char NotificationRenderer::proximityAlertMessage[256] = {0};
uint32_t NotificationRenderer::proximityAlertUntil = 0;
...
void NotificationRenderer::drawBannercallback(OLEDDisplay *display, OLEDDisplayUiState *state)
{
    ...
    // Proximity alert overlay — drawn independently so it doesn't conflict with text_input
    // or the banner overlay.  Times out via proximityAlertUntil.
    drawProximityAlertOverlay(display, state);
}
...
void NotificationRenderer::drawTextInput(OLEDDisplay *display, OLEDDisplayUiState *state)
{
    if (virtualKeyboard) {
        if (inEvent.inputEvent != INPUT_BROKER_NONE) {
            // 1. T9 popup gets first crack
            bool handled = OnScreenKeyboardModule::instance().handleT9Event(inEvent);
            // 2. VirtualKeyboard navigation
            if (!handled)
                handled = OnScreenKeyboardModule::processVirtualKeyboardInput(inEvent, virtualKeyboard);
            // 3. ANYKEY = insert character (e.g. space from hardware keypad)
            if (!handled && inEvent.inputEvent == INPUT_BROKER_ANYKEY && inEvent.kbchar != 0) {
                std::string text = virtualKeyboard->getInputText();
                text += static_cast<char>(inEvent.kbchar);
                virtualKeyboard->setInputText(text);
                handled = true;
            }
            // 4. BACK = backspace
        ...
        // Check T9 popup timeout after event processing (event may have reset t9LastMs)
        OnScreenKeyboardModule::instance().updateT9Timeout();

        // Check for keyboard timeout and auto-exit if needed
        ...
        // Draw popups managed by OnScreenKeyboardModule
        OnScreenKeyboardModule::instance().drawPopupOverlay(display);

#ifdef SHIPWRECKED
        // Snapshot PCA9555 state before the ~324ms partial refresh, so we can
        // detect new key presses that arrive during the refresh window (when
        // runOnce() / OSThread polling isn't running).
        uint16_t pcaPre = pca9555Kb ? pca9555Kb->readInputs() : 0xFFFF;
        EInkDisplay::triggerPartial(0, 0, display->getWidth(), display->getHeight());
        uint16_t pcaPost = pca9555Kb ? pca9555Kb->readInputs() : 0xFFFF;

        // HIGH→LOW transitions during the refresh = new presses.
        uint16_t fresh = (pcaPre ^ pcaPost) & ~pcaPost;
        if (fresh) {
            for (int i = 0; i < 16 && fresh; i++) {
                if (fresh & (1 << i)) {
                    int tbl = pca9555Kb->t9TableForIndex(i);
                    if (tbl >= 0 && OnScreenKeyboardModule::instance().isT9Active() &&
                        tbl == OnScreenKeyboardModule::instance().getT9ActiveTable()) {
                        OnScreenKeyboardModule::instance().t9CyclePublic(1);
                        OnScreenKeyboardModule::instance().touchT9Timeout();
                    }
                    fresh &= ~(1 << i);
                }
            }
...
void NotificationRenderer::drawProximityAlertOverlay(OLEDDisplay *display, OLEDDisplayUiState *state)
{
    if (proximityAlertMessage[0] == '\0' || (proximityAlertUntil > 0 && millis() > proximityAlertUntil)) {
        if (proximityAlertUntil > 0 && millis() > proximityAlertUntil)
            proximityAlertMessage[0] = '\0';
        return;
    }

    // Small banner at the top of the screen
    constexpr uint16_t bannerH = 16;
    display->setColor(BLACK);
    display->fillRect(0, 0, display->getWidth(), bannerH);
    display->setColor(WHITE);
    display->drawRect(0, 0, display->getWidth(), bannerH);

    display->setFont(FONT_SMALL);
    display->setTextAlignment(TEXT_ALIGN_LEFT);
    int16_t x = 4, y = (bannerH - FONT_HEIGHT_SMALL) / 2;
    display->drawString(x, y, proximityAlertMessage);
}
